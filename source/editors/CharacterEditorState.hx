...
class CharacterEditorState extends MusicBeatState
{
	...
	var renderTag:FlxSpriteGroup;

	var nameTag:FlxSprite;
	var renderName:FlxText;
	var renderSqr:FlxSprite;
	...
	override function create()
	{
		...
		renderTag = new FlxSpriteGroup(FlxG.width/2 + 50, FlxG.height - 100);
		renderTag.cameras = [camHUD];

		nameTag = new FlxSprite(0, 0).loadGraphic(Paths.image('nametag'));
		nameTag.color = FlxColor.GRAY;
		nameTag.antialiasing = ClientPrefs.globalAntialiasing;
		nameTag.cameras = [camHUD];
		renderTag.add(nameTag);

		renderName = new FlxText(0, 0, "FACE", 20);
		renderName.setFormat(Paths.font("FOT-RodinNTLGPro-UB.otf"), 15, 0xfff1f1f1, CENTER);
		updateRenderName("FACE");
		renderName.antialiasing = ClientPrefs.globalAntialiasing;
		renderTag.cameras = [camHUD];
		renderTag.add(renderName);

		renderSqr = new FlxSprite(0, 0).loadGraphic(Paths.image('render square'));
		//renderSqr.setPosition(nameTag.x + nameTag.width - 185, nameTag.y - nameTag.height/2 - renderSqr.height/2 - 15);
		renderSqr.color = FlxColor.GRAY;
		renderSqr.cameras = [camHUD];
		renderSqr.antialiasing = ClientPrefs.globalAntialiasing;
		renderTag.add(renderSqr);

		render = new FlxSprite(renderSqr.x, renderSqr.y);
		render.cameras = [camHUD];
		render.antialiasing = ClientPrefs.globalAntialiasing;
		renderTag.add(render);
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				character.renderImage = parsedJson.renderimage;
				character.renderName = parsedJson.rendername;
	...
	var renderImageInputText:FlxUIInputText;
	var renderNameInputText:FlxUIInputText;
	...
	function addCharacterUI() {
		...
		renderImageInputText = new FlxUIInputText(15, healthColorStepperR.y + 40, 75, char.renderImage, 8);
		renderNameInputText = new FlxUIInputText(positionCameraXStepper.x, renderImageInputText.y, 75, char.renderName, 8);
		...
		tab_group.add(new FlxText(15, renderImageInputText.y - 18, 0, 'Render file name:'));
		tab_group.add(new FlxText(renderNameInputText.x, renderNameInputText.y - 18, 0, 'Render display name:'));
		...
		tab_group.add(renderImageInputText);
		tab_group.add(renderNameInputText);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if (sender == renderImageInputText) {
				char.renderImage = renderImageInputText.text;

				updateTag();
			}
			else if (sender == renderNameInputText) {
				char.renderName = renderNameInputText.text;
				updateRenderName(renderNameInputText.text);
			}
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			renderImageInputText.text = char.renderImage;
			renderNameInputText.text = char.renderName;
			updateTag();
			reloadAnimationDropDown();
			updatePresence();
		}
	...
	function updateRenderName(?name:String) {
		if (name != null)
			name = name.toUpperCase();
		else
			name = renderName.text.toUpperCase();
		renderName.text = name;
		renderName.scale.x = 1;
		renderName.updateHitbox();
		
		if (char.isPlayer) {
			renderName.setPosition(nameTag.x + nameTag.width/2 - renderName.width/2 - 30, nameTag.y + nameTag.height/2 - renderName.height/2 + 2);
			while (renderName.x < nameTag.x + 5) {
				renderName.setGraphicSize(Std.int(renderName.width*0.99), Std.int(renderName.height));
				renderName.updateHitbox();
				renderName.setPosition(nameTag.x + nameTag.width/2 - renderName.width/2 - 30, nameTag.y + nameTag.height/2 - renderName.height/2 + 2);
			}
		}
			...
			renderName.setPosition(nameTag.x + nameTag.width/2 - renderName.width/2 + 30, nameTag.y + nameTag.height/2 - renderName.height/2 + 2);
			while (renderName.x + renderName.width > nameTag.x + nameTag.width - 5) {
				renderName.setGraphicSize(Std.int(renderName.width*0.99), Std.int(renderName.height));
				renderName.updateHitbox();
				renderName.setPosition(nameTag.x + nameTag.width/2 - renderName.width/2 + 30, nameTag.y + nameTag.height/2 - renderName.height/2 + 2);
			}
	...
	function updateTag() {
		nameTag.flipX = char.isPlayer;
		updateRenderName(char.renderName);
		renderSqr.flipX = char.isPlayer;
		render.flipX = char.isPlayer;

		if (char.isPlayer) {
			renderSqr.setPosition(nameTag.x + nameTag.width - 185, nameTag.y - nameTag.height/2 - renderSqr.height/2 - 15);
			nameTag.color = renderSqr.color = p1Color;
		}
		else {
			renderSqr.setPosition(nameTag.x - renderSqr.width + 185, nameTag.y - nameTag.height/2 - renderSqr.height/2 - 15);
			nameTag.color = renderSqr.color = p2Color;
		}
		render.setPosition(renderSqr.x, renderSqr.y);

		var name:String = char.renderImage;
		if(!Paths.fileExists('images/renders/' + name + '.png', IMAGE))
			name = "face";
		render.loadGraphic(Paths.image('renders/' + name));
		render.antialiasing = ClientPrefs.globalAntialiasing && !name.endsWith("-pixel");
	}
	...
	override function update(elapsed:Float)
	{
		...
		var inputTexts:Array<FlxUIInputText> = [animationInputText, imageInputText, healthIconInputText, animationNameInputText, animationIndicesInputText, renderImageInputText, renderNameInputText];
	...
	function saveCharacter() {
		var json = {
			...
			"renderimage": char.renderImage,
			"rendername": char.renderName
		};
